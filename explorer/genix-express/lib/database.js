...
function get_market_data(market, cb) {
  switch(market) {
    ...
    case 'coinexchange':
      ...
      break;
    case 'swiftex':
      swiftex.get_data(settings.markets.coin, settings.markets.exchange, function(err, obj){
        return cb(err, obj);
      });
      break;
      case 'altmarket':
        altmarket.get_data(settings.markets.coin, settings.markets.exchange, function(err, obj){
          return cb(err, obj);
        });
        break;
    default:
...
module.exports = {
  ...
  update_markets_db: function(market, cb) {
    console.log('attempting to update: ' + market)
    get_market_data(market, function (err, obj) {
      if (err == null) {
        console.log('updating market stats for: ' + market)
        Markets.update({market:market}, {
          ...
          if ( market == settings.markets.default ) {
            console.log('Updating last price for ' + market + ' last price data: ' + obj.stats.last)
